...
function populateCountryFilters() {
    ...
    const operatorsSorted = [...operators].sort();
...
function renderEquipmentGrid() {
    ...
    if (practiceState.currentCategory !== 'all') {
        filteredEquipment = filteredEquipment.filter(eq => eq.type === practiceState.currentCategory);
    }
...
function createEquipmentCard(equipment) {
    ...
    card.innerHTML = `
        ...
        <img class="equipment-card-image" src="${equipment.image}" alt="${equipment.name}" onerror="this.src='https://placehold.co/200x220/1a1a1a/00ff88?text=${encodeURIComponent(equipment.name)}'">
...
function openPracticeModal(equipment) {
    ...
    if (equipment.imageCredit) {
        if (typeof equipment.imageCredit === 'string') {
            modalCredit.textContent = equipment.imageCredit;
        } else {
            // Handle object format: { author, source, license, url }
            const credit = equipment.imageCredit;
            const creditText = `Photo: ${credit.author} / ${credit.source} / ${credit.license}`;
            if (credit.url) {
                modalCredit.innerHTML = `<a href="${credit.url}" target="_blank" rel="noopener">${creditText}</a>`;
            } else {
